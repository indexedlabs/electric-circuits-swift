...
# dependency. The only 0.2.1 tag created here belongs to a throwaway clone and is deleted with it.
...
RELEASE_REPOSITORY_URL="file://$release_repo" perl -0pi -e '
  BEGIN {
    ...
    $new = qq{.package(url: "$ENV{RELEASE_REPOSITORY_URL}", exact: "0.2.1"),};
  }
